...
class ProImageEditor extends StatefulWidget with SimpleConfigsAccess, SimpleCallbacksAccess {
...
class ProImageEditorState extends State<ProImageEditor>
  ...
  final LayerInteractionManager layerInteractionManager = LayerInteractionManager();
  ...
  Layer? get _activeLayer => activeLayers.length > selectedLayerIndex && selectedLayerIndex >= 0
      ? activeLayers[selectedLayerIndex]
      : null;

  /// Get the list of layers from the current image editor changes.
  List<Layer> get activeLayers => stateManager.stateHistory[stateManager.position].layers;
  ...
  bool get canRedo => stateManager.position < stateManager.stateHistory.length - 1;
  ...
  void initState() {
    ...
    _controllers.screenshot.generateOnlyThumbnail = callbacks.onThumbnailGenerated != null;
    ...
    layerInteractionManager.scaleDebounce = Debounce(const Duration(milliseconds: 100));
    ...
    if (helperLines.hitVibration) {
      Vibration.hasVibrator().then((hasVibrator) {
        ...
        if (layerInteractionManager.deviceCanVibrate) {
          Vibration.hasCustomVibrationsSupport().then((hasCustomVibrationsSupport) {
            layerInteractionManager.deviceCanCustomVibrate = hasCustomVibrationsSupport ?? false;
          });
        }
      });
    }
  ...
  void addHistory({
    ...
    stateHistory.add(
      EditorStateHistory(
        ...
        layers: layers ?? (newLayer != null ? [...activeLayerList, newLayer] : activeLayerList),
    ...
    if (!blockCaptureScreenshot) {
      ...
      _controllers.screenshot.addEmptyScreenshot(screenshots: stateManager.screenshots);
    }
  ...
  void removeLayer(int layerPos, {Layer? layer}) {
    int oldIndex =
        activeLayers.indexWhere((element) => element.id == (layer?.id ?? _tempLayer!.id));
    if (oldIndex >= 0) {
      stateHistory[stateManager.position].layers[oldIndex] =
          _layerCopyManager.copyLayer(_tempLayer ?? layer!);

      mainEditorCallbacks?.handleRemoveLayer(stateHistory[stateManager.position].layers[oldIndex]);
    }

    var layers = _layerCopyManager.copyLayerList(activeLayers)..removeAt(layerPos);
  ...
  void _updateTempLayer() {
    ...
    int oldIndex = oldLayers.indexWhere((element) => element.id == _tempLayer!.id);
    if (oldIndex >= 0) {
  ...
  void _calcAppBarHeight() {
    double? renderedBottomBarHeight = _bottomBarKey.currentContext?.size?.height;
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    bool beforeShowHorizontalHelperLine = layerInteractionManager.showHorizontalHelperLine;
    bool beforeShowVerticalHelperLine = layerInteractionManager.showVerticalHelperLine;
    bool beforeShowRotationHelperLine = layerInteractionManager.showRotationHelperLine;

    void checkUpdateHelperLineUI() {
      if (beforeShowHorizontalHelperLine != layerInteractionManager.showHorizontalHelperLine ||
          beforeShowVerticalHelperLine != layerInteractionManager.showVerticalHelperLine ||
          beforeShowRotationHelperLine != layerInteractionManager.showRotationHelperLine) {
    ...
    if (layerInteractionManager.rotateScaleLayerSizeHelper != null) {
      layerInteractionManager
        ...
        ..calculateInteractiveButtonScaleRotate(
          ...
          editorScaleOffset: _interactiveViewer.currentState?.offset ?? Offset.zero,
        );
    ...
    double editorScaleFactor = _interactiveViewer.currentState?.scaleFactor ?? 1.0;
  ...
  void _selectLayerAfterHeroIsDone(String id) {
    if (layerInteractionManager.layersAreSelectable(configs) && layerInteraction.initialSelected) {
      /// Skip one frame to ensure captured image in separate thread will not
      /// capture the border.
              ...
              child: Stack(
                ...
                children: [
                  Positioned(
                    ...
                    child: Center(
                      child: Container(
                        width: imageEditorTheme.subEditorPage.enforceSizeFromMainEditor
                            ? sizesManager.editorSize.width
                            : null,
                        height: imageEditorTheme.subEditorPage.enforceSizeFromMainEditor
                            ? sizesManager.editorSize.height
                            : null,
                        clipBehavior: Clip.hardEdge,
                        decoration: BoxDecoration(
                          borderRadius: imageEditorTheme.subEditorPage.borderRadius,
                        ),
                        child: page,
                      ),
                    ),
  ...
  void openPaintingEditor() async {
    List<PaintingLayerData>? paintingLayers = await openPage<List<PaintingLayerData>>(
    ...
    await openPage<TransformConfigs?>(
      CropRotateEditor.autoSource(
        ...
        initConfigs: CropRotateEditorInitConfigs(
          ...
          transformConfigs: stateManager.stateHistory[stateManager.position].transformConfigs,
          mainImageSize: sizesManager.decodedImageSize,
          mainBodySize: sizesManager.bodySize,
          enableFakeHero: true,
          appliedBlurFactor: stateManager.activeBlur,
  ...
  void openEmojiEditor() async {
    ...
    final effectiveBoxConstraints =
        imageEditorTheme.emojiEditor.editorBoxConstraintsBuilder?.call(context, configs) ??
            imageEditorTheme.editorBoxConstraintsBuilder?.call(context, configs);

    ThemeDraggableSheet sheetTheme = imageEditorTheme.emojiEditor.themeDraggableSheet;
        ...
        builder: (BuildContext context) {
          if (!useDraggableSheet) {
            return ConstrainedBox(
              constraints: effectiveBoxConstraints ??
                  BoxConstraints(maxHeight: 300 + MediaQuery.of(context).viewInsets.bottom),
  ...
  void openStickerEditor() async {
    ...
    ServicesBinding.instance.keyboard.removeHandler(_onKeyEvent);
    final effectiveBoxConstraints =
        imageEditorTheme.stickerEditor.editorBoxConstraintsBuilder?.call(context, configs) ??
            imageEditorTheme.editorBoxConstraintsBuilder?.call(context, configs);
  ...
  void doneEditing() async {
    ...
    if (mainEditorConfigs.transformSetup != null && stateManager.position == 0) {
      addHistory();
    }
  ...
  Future<Uint8List> captureEditorImage() async {
    ...
    bool useOriginalImage = !hasChanges && imageGenerationConfigs.enableUseOriginalBytes;
    ...
    return await _controllers.screenshot.captureFinalScreenshot(
          imageInfos: _imageInfos!,
          backgroundScreenshot: useOriginalImage ? null : stateManager.activeScreenshot,
          originalImageBytes: useOriginalImage ? await editorImage.safeByteArray(context) : null,
        ) ??
        Uint8List.fromList([]);
  }
  ...
  void importStateHistory(ImportStateHistory import) {
      ...
      for (EditorStateHistory el in import.stateHistory) {
        for (Layer layer in el.layers) {
          if (import.configs.recalculateSizeAndPosition) {
            ...
            double scaleWidth = currentImageSize.width / lastDecodedImageSize.width;
            double scaleHeight = currentImageSize.height / lastDecodedImageSize.height;
    ...
    if (import.configs.mergeMode == ImportEditorMergeMode.replace) {
      stateManager
        ..screenshots = []
        ..position = import.editorPosition + (import.stateHistory.isEmpty ? 0 : 1)
        ..stateHistory = [
          EditorStateHistory(
            transformConfigs: TransformConfigs.empty(),
            blur: 0,
  ...
  Widget build(BuildContext context) {
    ...
    return RecordInvisibleWidget(
      ...
      child: ExtendedPopScope(
        canPop: disablePopScope || stateManager.position <= 0 || _processFinalImage,
        onPopInvokedWithResult: (didPop, result) {
          if (!didPop && !disablePopScope && stateManager.position > 0 && !_processFinalImage) {
            closeWarning();
          }
  ...
  PreferredSizeWidget? _buildAppBar() {
    if (customWidgets.mainEditor.appBar != null) {
      return customWidgets.mainEditor.appBar!.call(this, _rebuildController.stream);
    }

    return selectedLayerIndex >= 0 && configs.layerInteraction.hideToolbarOnInteraction
        ? null
        : AppBar(
  ...
  Widget _buildInteractiveContent() {
    return Center(
      child: Stack(
        children: [
          Padding(
            padding: selectedLayerIndex >= 0 && configs.layerInteraction.hideToolbarOnInteraction
                ? EdgeInsets.only(
            ...
            child: ExtendedInteractiveViewer(
              ...
              onInteractionStart: (details) {
                ...
                layerInteractionManager.freeStyleHighPerformanceEditorZoom =
                    (paintEditorConfigs.freeStyleHighPerformanceMoving ?? !isDesktop) ||
                        (paintEditorConfigs.freeStyleHighPerformanceScaling ?? !isDesktop);

                _controllers.uiLayerCtrl.add(null);
              },
              onInteractionUpdate: callbacks.mainEditorCallbacks?.onEditorZoomScaleUpdate,
          ...
          if (imageGenerationConfigs.captureOnlyBackgroundImageArea)
            Hero(
              ...
              child: CustomPaint(
                foregroundPainter: imageGenerationConfigs.captureOnlyBackgroundImageArea
                    ? CropLayerPainter(
                        opacity: imageEditorTheme.outsideCaptureAreaLayerOpacity,
          ...
          if (customWidgets.mainEditor.bodyItems != null)
            ...customWidgets.mainEditor.bodyItems!(this, _rebuildController.stream),
        ],
      ),
    );
  }
                          ...
                          children: <Widget>[
                            ...
                            if (stickerEditorConfigs?.enabled == true)...[
                              FlatIconTextButton(
                                ...
                                icon: icons.stickerEditor.bottomNavBar ?? Icon(
                                  Icons.sticky_note_2,
                                  size: bottomIconSize,
                                  color: Colors.white,
                                ),
          ...
          builder: (context, resetLayerSnapshot) {
            ...
            return RepaintBoundary(
              child: ExtendedMouseRegion(
                ...
                onHover: isDesktop
                    ? (event) {
                        bool hasHit = activeLayers.indexWhere(
                                (element) => element is PaintingLayerData && element.item.hit) >=
                            0;

                        MouseCursor activeCursor = _mouseCursorsKey.currentState!.currentCursor;
                        MouseCursor moveCursor = imageEditorTheme.layerInteraction.hoverCursor;
                        ...
                        return Stack(
                          children: activeLayers.asMap().entries.map((entry) {
                            ...
                            return LayerWidget(
                              ...
                              selected: layerInteractionManager.selectedLayerId == layerItem.id,
                              ...
                              onTap: (layer) async {
                                if (layerInteractionManager.layersAreSelectable(configs)) {
                                  layerInteractionManager.selectedLayerId =
                                      layer.id == layerInteractionManager.selectedLayerId
                                          ? ''
                                          : layer.id;
                                  _checkInteractiveViewer();
                ...
                if (helperLines.showVerticalLine)
                  Align(
                    ...
                    child: AnimatedContainer(
                      ...
                      width: layerInteractionManager.showVerticalHelperLine ? lineH : 0,
                      height: screenH,
                ...
                if (helperLines.showHorizontalLine)
                  Align(
                    ...
                    child: AnimatedContainer(
                      ...
                      duration: Duration(milliseconds: duration),
                      width: screenW,
                      height: layerInteractionManager.showHorizontalHelperLine ? lineH : 0,
                  ...
                  Positioned(
                    ...
                    child: FractionalTranslation(
                      ...
                      child: Transform.rotate(
                        ...
                        child: AnimatedContainer(
                          duration: Duration(milliseconds: duration),
                          width: layerInteractionManager.showRotationHelperLine ? lineH : 0,
                          height: screenH * 2,
                  ...
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      color: layerInteractionManager.hoverRemoveBtn
                          ? imageEditorTheme.layerInteraction.removeAreaBackgroundActive
                          : imageEditorTheme.layerInteraction.removeAreaBackgroundInactive,
